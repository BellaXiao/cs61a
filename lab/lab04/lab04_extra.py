...
def print_board(board, max_rows, max_cols):
    ...
    for row in range(max_rows):
        ...
        for col in range(max_cols):
            ...
            result_row += piece + " "
        print(result_row.strip())
    # add a row of column index to make play easier
    col_index = ""
    for i in range(max_cols):
        col_index += str(i) + " "
    print(col_index.strip())
...
def check_win_row(board, max_rows, max_cols, num_connect, row, player):
    ...
    for col in range(max_cols):
        piece = get_piece(board, row, col)
        result_row += piece
        if player*num_connect in result_row:
            return True
    return False
...
def check_win_column(board, max_rows, max_cols, num_connect, col, player):
    ...
    for row in range(max_rows):
        piece = get_piece(board, row, col)
        result_col += piece
        if player*num_connect in result_col:
            return True
    return False
