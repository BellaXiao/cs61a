...
def take(src, allowed_characters):
	""" Extract characters in allowed_characters from src before encounting
	the first not allow character.
	Return a new result forming by those consective allowed characters.
	And also update src starting with the first character not in allowed_characters.
	"""
	result = ''
...
def next_token(src):
	# removve the white space in front first
	take(src, WHITESPACE)
	c = src.current()
	if c is None:
		return None
...
############
    ...
    def read_expr(src):
    	...
    	if token is None:
    		return raise SyntaxError('Incomplete expression')
    ...
    def read_call_expr(src, operator):
    	"""Get operands list from src, combined with operator,
    	return CallExpr(operator, operands)
    	"""
    	while src.current() == '(':
    		src.remove_front()
    ...
    def read_comma_separated(src, reader):
    	"""read a comma separated token list using reader function,
    	return a reader(token) list without comma in the list.
    	Used both for lambada expression parameter list reading, as well as
    	callexpr operands list reading.
    	return a list.
    	"""
    	if src.current() in (':', ')'):
    		return []
    ...
    def read_param(src):
    	"""Get the parameters for lambda function, if it's a Name type token,
    	return it from src, and also move it from src.
    	If it's not a Name type, raise SyntaxError.
    	"""
    	token = src.remove_front()
    	if is_name(token):
    		return token
