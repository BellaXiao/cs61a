...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        return (self.place.ant is not None and 
                self.place.ant.block_path!=False)
        # END Problem 7
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        count = 0
        while place != hive:
            if self.min_range<=count<=self.max_range and random_or_none(place.bees):
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if not self.real_queen:
            ...
            while place:
                if place.ant:
                    if not place.ant.damage_is_doubled:
                        place.ant.damage *= 2
                        place.ant.damage_is_doubled = True
                    if isinstance(place.ant, BodyguardAnt):
                        if place.ant.contained_ant and not place.ant.contained_ant.damage_is_doubled:
                            place.ant.contained_ant.damage *= 2
                            place.ant.contained_ant.damage_is_doubled = True
                place = place.exit
    ...
    def reduce_armor(self, amount):
        ...
        "*** YOUR CODE HERE ***"
        self.damage -= amount
        if self.damage <=0 and self.real_queen:
            bees_win()
        # END Problem 13
...
def apply_effect(effect, bee, duration):
    ...
    cur_action = effect(bee.action, bee)
    def action(colony):
        nonlocal duration
        if duration > 0:
            cur_action(colony)
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, hive):
        ...
        while place != hive:
            for bee in place.bees:
                result[bee] = distance
            if place.ant:
                if distance==0 and not isinstance(place.ant, LaserAnt):
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem OPTIONAL
        reduce1 = 0.2*distance
        reduce2 = self.insects_shot * 0.05
        damage = max(2-reduce1-reduce2,0)
        return damage
