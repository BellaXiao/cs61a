...
def is_swap(player_score, opponent_score):
    ...
    tens_1 = player_score // 10 % 10
    ones_2 = opponent_score % 10
    tens_2 = opponent_score // 10 % 10

    return abs(ones_2 - tens_2) == abs(ones_1-tens_1)
    # END PROBLEM 4
...
def announce_highest(who, previous_high=0, previous_score=0):
    ...
    def say(score0, score1):
        ...
        if score[who] - previous_score > pre_high:
            print("{} point(s)! That's the biggest gain yet for Player {}".format(score[who] - previous_score, who))
            pre_high = score[who] - previous_score
        return announce_highest(who, pre_high, score[who])
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    for num_rolls in range(1,11):
        ...
        ave_num_rolls = fn(num_rolls, dice)
        if ave_max < ave_num_rolls:
            ave_max = ave_num_rolls
            num_rolls_opt = num_rolls
    return num_rolls_opt
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    ones = opponent_score % 10
    tens = opponent_score // 10
    temp = max(1, 2 * tens - ones)
    if is_swap(temp + score, opponent_score):
        if temp+score < opponent_score:
            return 0
        else:
            return num_rolls
    elif temp >= margin:
