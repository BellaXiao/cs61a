...
def find_predictor(user, restaurants, feature_fn):
    ...
    x_mean = mean(xs)
    y_mean = mean(ys)
    x_diff_list = [x-x_mean for x in xs]
    y_diff_list = [y-y_mean for y in ys]
    S_xx = sum([d**2 for d in x_diff_list])  
    S_yy = sum([d**2 for d in y_diff_list])
    S_xy =sum([x*y for x,y in zip(x_diff_list,y_diff_list)])
...
def best_predictor(user, restaurants, feature_fns):
    ...
    # BEGIN Question 8
    "*** YOUR CODE HERE ***"
    fearture_fn = max(feature_fns,key=lambda fn: find_predictor(user, reviewed, fn)[1])
    return find_predictor(user, reviewed, fearture_fn)[0]
