...
# replace the above for problem 20
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('Inconsistent length of {0} and {1}.'.format(formals, vals))
...
# for problem 20, use tail scheme_eval
def do_and_form(expressions, env):
    """Evaluate a (short-circuited) and form."""
    # BEGIN PROBLEM 13
    if expressions is nil:
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        check_form(pair, 2, 2)
        formal = pair.first
        val = scheme_eval(pair.second.first, env)
